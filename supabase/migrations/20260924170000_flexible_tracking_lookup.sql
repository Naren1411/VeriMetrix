/* Allow applicants to track with either identifier; both remain supported together. */
...
AS $$
  SELECT COALESCE(
    (
      ...
      WHERE (NULLIF(trim(application_number_input), '') IS NULL OR upper(a.application_number) = upper(trim(application_number_input)))
        AND (NULLIF(trim(email_input), '') IS NULL OR lower(trim(a.email)) = lower(trim(email_input)))
