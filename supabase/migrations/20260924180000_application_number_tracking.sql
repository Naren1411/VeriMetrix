/* Restore application-number-only public tracking for the prototype workflow. */

DROP FUNCTION IF EXISTS public.public_track_application(text, text);
...
AS $$
  SELECT COALESCE(
    (
      ...
      WHERE upper(a.application_number) = upper(trim(application_number_input))
    ), '{}'::jsonb
  );
$$;
